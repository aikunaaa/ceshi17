...
if (isset($_POST['submit'])) {
    ...
    $blacklist = array("select", "insert", "update", "delete", "drop", "union", " ", "or", "where", "from");

    foreach ($blacklist as $keyword) {
        $username = preg_replace('/\s*' . preg_quote($keyword, '/') . '\s*/i', '', $username);
    }

    foreach ($blacklist as $keyword) {
        $password = preg_replace('/\s*' . preg_quote($keyword, '/') . '\s*/i', '', $password);
    }
